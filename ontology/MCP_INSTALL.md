# Finance MCP Install Guide

`finance`는 OpenTax 세금 온톨로지와 분리된 금융상품 온톨로지를 함께 다루는
stdio 방식 MCP 서버입니다. MCP를 지원하는 LLM 클라이언트는
...
opentax-generate
opentax-validate
finance-build-ontology
finance-import-products
finance-validate
```

기존 설치 문서와 클라이언트 호환을 위해 `opentax-mcp`, `tax-ontology-mcp`,
...
[mcp_servers.finance.tools.finance_add_or_update_item]
approval_mode = "approve"

[mcp_servers.finance.tools.finance_patch_item]
approval_mode = "approve"

[mcp_servers.finance.tools.finance_delete_custom_item]
approval_mode = "approve"
```
...
배포 이름은 `finance-mcp`이며, 배포 후 ChatGPT 커넥터에는 다음 형태의 URL을
등록합니다.
...
- `finance_add_or_update_item`
- `finance_patch_item`
- `finance_delete_custom_item`

기존 연결 호환을 위해 `opentax_*`, `tax_ontology_*` alias도 계속 제공합니다.
...
```sh
cd /path/to/TaxMeter
zip -r finance-mcp.zip ontology \
