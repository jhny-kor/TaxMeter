---
id: "corporate.support.employment-maintenance-credit"
title: "고용유지 중소기업 등에 대한 과세특례"
type: "corporate-tax-support"
description: "임금 감소와 시간당 임금 보전액 등을 기준으로 적용되는 세액공제입니다."
basis_year: 2026
parents: ["category.corporate-tax-supports"]
children: []
related: []
terms: ["term.tax-credit", "term.tax-reduction"]
deadlines: []
sources: ["source.nts.corporate-tax.reliefs"]
criteria: [{"label": "고용유지 임금감소분", "basis": "임금감소액 또는 보전액", "condition": "고용유지 요건 충족", "rate_percent": 50, "rate_label": "공제율", "source": "source.nts.corporate-tax.reliefs", "amount_formula": "임금감소액 또는 보전액 × 공제율 50%"}]
law_reference: ""
tags: ["corporate-tax-support"]
---

# 고용유지 중소기업 등에 대한 과세특례

임금 감소와 시간당 임금 보전액 등을 기준으로 적용되는 세액공제입니다.

## 기준 내역

- **고용유지 임금감소분**: 기준항목: 임금감소액 또는 보전액; 조건: 고용유지 요건 충족; 공제율: 50%; 금액·적용 산식: 임금감소액 또는 보전액 × 공제율 50%; 출처: [[90_Sources/법인세 공제감면|법인세 공제감면]]

## 상위 항목

- [[20_Deductions/CorporateTaxSupports/법인세 공제·감면 지원제도|법인세 공제·감면 지원제도]]

## 관련 용어

- [[40_Terms/세액공제|세액공제]]
- [[40_Terms/세액감면|세액감면]]

## 근거·출처

- [[90_Sources/법인세 공제감면|법인세 공제감면]]: https://www.nts.go.kr/nts/cm/cntnts/cntntsView.do?cntntsId=7987&mi=6561

## 온톨로지 ID

`corporate.support.employment-maintenance-credit`
