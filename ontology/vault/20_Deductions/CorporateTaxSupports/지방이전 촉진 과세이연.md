---
id: "corporate.support.local-relocation-deferral"
title: "지방이전 촉진 과세이연"
type: "corporate-tax-support"
description: "지방이전 등을 촉진하기 위한 양도차익 등의 손금산입 또는 익금불산입 지원입니다."
basis_year: 2026
parents: ["category.corporate-tax-supports"]
children: []
related: []
terms: ["term.tax-credit", "term.tax-reduction"]
deadlines: []
sources: ["source.nts.corporate-tax.reliefs"]
criteria: [{"label": "지방이전 과세이연", "basis": "지방이전 관련 양도차익 등", "condition": "지방이전 촉진 요건 충족", "benefit": "손금산입 또는 익금불산입으로 과세이연", "source": "source.nts.corporate-tax.reliefs", "amount_applicability": "정액 금액 기준 없음"}]
law_reference: ""
tags: ["corporate-tax-support"]
---

# 지방이전 촉진 과세이연

지방이전 등을 촉진하기 위한 양도차익 등의 손금산입 또는 익금불산입 지원입니다.

## 기준 내역

- **지방이전 과세이연**: 기준항목: 지방이전 관련 양도차익 등; 조건: 지방이전 촉진 요건 충족; 금액 기준 여부: 정액 금액 기준 없음; 혜택: 손금산입 또는 익금불산입으로 과세이연; 출처: [[90_Sources/법인세 공제감면|법인세 공제감면]]

## 상위 항목

- [[20_Deductions/CorporateTaxSupports/법인세 공제·감면 지원제도|법인세 공제·감면 지원제도]]

## 관련 용어

- [[40_Terms/세액공제|세액공제]]
- [[40_Terms/세액감면|세액감면]]

## 근거·출처

- [[90_Sources/법인세 공제감면|법인세 공제감면]]: https://www.nts.go.kr/nts/cm/cntnts/cntntsView.do?cntntsId=7987&mi=6561

## 온톨로지 ID

`corporate.support.local-relocation-deferral`
