---
id: "corporate.support.rnd-grant-deferral"
title: "연구개발 관련 출연금등의 과세이연"
type: "corporate-tax-support"
description: "연구개발출연금 등에 대한 익금불산입 지원입니다."
basis_year: 2026
parents: ["category.corporate-tax-supports"]
children: []
related: []
terms: ["term.tax-credit", "term.tax-reduction"]
deadlines: []
sources: ["source.nts.corporate-tax.reliefs"]
criteria: [{"label": "연구개발출연금 과세이연", "basis": "연구개발출연금 등", "condition": "연구개발 자산 취득 또는 비용 지출", "benefit": "익금불산입 후 사용 시점에 익금산입", "source": "source.nts.corporate-tax.reliefs", "amount_applicability": "정액 금액 기준 없음"}]
law_reference: ""
tags: ["corporate-tax-support"]
---

# 연구개발 관련 출연금등의 과세이연

연구개발출연금 등에 대한 익금불산입 지원입니다.

## 기준 내역

- **연구개발출연금 과세이연**: 기준항목: 연구개발출연금 등; 조건: 연구개발 자산 취득 또는 비용 지출; 금액 기준 여부: 정액 금액 기준 없음; 혜택: 익금불산입 후 사용 시점에 익금산입; 출처: [[90_Sources/법인세 공제감면|법인세 공제감면]]

## 상위 항목

- [[20_Deductions/CorporateTaxSupports/법인세 공제·감면 지원제도|법인세 공제·감면 지원제도]]

## 관련 용어

- [[40_Terms/세액공제|세액공제]]
- [[40_Terms/세액감면|세액감면]]

## 근거·출처

- [[90_Sources/법인세 공제감면|법인세 공제감면]]: https://www.nts.go.kr/nts/cm/cntnts/cntntsView.do?cntntsId=7987&mi=6561

## 온톨로지 ID

`corporate.support.rnd-grant-deferral`
