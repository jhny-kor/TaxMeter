---
id: "corporate.support.enterprise-city-reduction"
title: "기업도시개발구역 창업기업 등 감면"
type: "corporate-tax-support"
description: "기업도시개발구역 창업 또는 사업장 신설 기업에 적용되는 감면입니다."
basis_year: 2026
parents: ["category.corporate-tax-supports"]
children: []
related: []
terms: ["term.tax-credit", "term.tax-reduction"]
deadlines: []
sources: ["source.nts.corporate-tax.reliefs"]
criteria: [{"label": "기업도시개발구역 창업·신설", "basis": "법인세", "condition": "기업도시개발구역 창업 또는 사업장 신설", "rate_percent_min": 50, "rate_percent_max": 100, "rate_label": "감면율", "source": "source.nts.corporate-tax.reliefs", "amount_formula": "법인세 × 감면율 50%~100%"}]
law_reference: ""
tags: ["corporate-tax-support"]
---

# 기업도시개발구역 창업기업 등 감면

기업도시개발구역 창업 또는 사업장 신설 기업에 적용되는 감면입니다.

## 기준 내역

- **기업도시개발구역 창업·신설**: 기준항목: 법인세; 조건: 기업도시개발구역 창업 또는 사업장 신설; 최저감면율: 50%; 최고감면율: 100%; 금액·적용 산식: 법인세 × 감면율 50%~100%; 출처: [[90_Sources/법인세 공제감면|법인세 공제감면]]

## 상위 항목

- [[20_Deductions/CorporateTaxSupports/법인세 공제·감면 지원제도|법인세 공제·감면 지원제도]]

## 관련 용어

- [[40_Terms/세액공제|세액공제]]
- [[40_Terms/세액감면|세액감면]]

## 근거·출처

- [[90_Sources/법인세 공제감면|법인세 공제감면]]: https://www.nts.go.kr/nts/cm/cntnts/cntntsView.do?cntntsId=7987&mi=6561

## 온톨로지 ID

`corporate.support.enterprise-city-reduction`
