---
id: "corporate.support.crisis-area-startup-reduction"
title: "위기지역 창업기업 감면"
type: "corporate-tax-support"
description: "위기지역 지정 또는 선포 기간에 창업하거나 사업장을 신설한 기업에 대한 감면입니다."
basis_year: 2026
parents: ["category.corporate-tax-supports"]
children: []
related: []
terms: ["term.tax-credit", "term.tax-reduction"]
deadlines: []
sources: ["source.nts.corporate-tax.reliefs"]
criteria: [{"label": "위기지역 창업기업", "basis": "법인세", "condition": "위기지역 지정 또는 선포 기간 창업·사업장 신설", "rate_percent_min": 50, "rate_percent_max": 100, "rate_label": "감면율", "note": "감면기간 및 지역별 요건 별도 확인", "source": "source.nts.corporate-tax.reliefs", "amount_formula": "법인세 × 감면율 50%~100%"}]
law_reference: ""
tags: ["corporate-tax-support"]
---

# 위기지역 창업기업 감면

위기지역 지정 또는 선포 기간에 창업하거나 사업장을 신설한 기업에 대한 감면입니다.

## 기준 내역

- **위기지역 창업기업**: 기준항목: 법인세; 조건: 위기지역 지정 또는 선포 기간 창업·사업장 신설; 최저감면율: 50%; 최고감면율: 100%; 금액·적용 산식: 법인세 × 감면율 50%~100%; 비고: 감면기간 및 지역별 요건 별도 확인; 출처: [[90_Sources/법인세 공제감면|법인세 공제감면]]

## 상위 항목

- [[20_Deductions/CorporateTaxSupports/법인세 공제·감면 지원제도|법인세 공제·감면 지원제도]]

## 관련 용어

- [[40_Terms/세액공제|세액공제]]
- [[40_Terms/세액감면|세액감면]]

## 근거·출처

- [[90_Sources/법인세 공제감면|법인세 공제감면]]: https://www.nts.go.kr/nts/cm/cntnts/cntntsView.do?cntntsId=7987&mi=6561

## 온톨로지 ID

`corporate.support.crisis-area-startup-reduction`
