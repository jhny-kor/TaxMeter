---
materialized_id: "support.local-gov.gov24.2000024"
title: "청소년 통합지원체계(청소년안전망) (인천광역시서해구시설관리공단)"
type: "support-program"
domain: "local-government-supports"
basis_year: 2026
reviewed_at: "2026-07-11"
source_export: "korea-local-government-supports-ontology-2026.json"
source_urls: ["https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList/benefitSrvcDtl?svcSeq=2000024&bnefType=all&svcId=O00027100001", "https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList"]
source_basis_dates: ["정부24 원문 수정일 2026-05-04", "수집일 2026-07-11", "2026-05-04 확인"]
tags: ["graph-materialized", "local-government-supports", "local-government-support", "gov24", "generated", "기타(상담)", "custom-overlay"]
---

# 청소년 통합지원체계(청소년안전망) (인천광역시서해구시설관리공단)

인천광역시서해구시설관리공단 관할 지자체 지원금입니다. 위기 청소년을 조기에 발견하고, 맞춤형 지원을 통해 보호와 복지를 제공하는 제도입니다.
교육·상담·복지·보호 서비스가 통합 제공되며, 필요에 따라 긴급 지원도 가능합니다.
청소년은 위기 상황에서 안전망을 확보하고, 건강하게 성장할 수 있습니다.

> Graph-only materialization입니다. 원본 데이터는 JSON export가 authoritative source입니다.

## Metadata
- Materialized ID: `support.local-gov.gov24.2000024`
- Type: `support-program`
- jurisdiction: 인천광역시서해구시설관리공단
- application_method: 온오프라인
- receiving_agency: 인천서구 청소년상담복지센터
- contact: 청소년상담복지센터(070-8177-4275)
- source_modified_at: 2026-05-04
- source_collected_at: 2026-07-11

## Relations
- parents: [[30_Supports/LocalGovernment/지자체 지원금|지자체 지원금]]
- terms: [[40_Terms/지자체 지원금|지자체 지원금]]
- sources: [[90_Sources/정부24 보조금24 전체 혜택|정부24 보조금24 전체 혜택]]

## Status Check
- https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList/benefitSrvcDtl?svcSeq=2000024&bnefType=all&svcId=O00027100001

## Source URLs
- https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList/benefitSrvcDtl?svcSeq=2000024&bnefType=all&svcId=O00027100001
- https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList
