---
materialized_id: "support.local-gov.gov24.2001602"
title: "초·중·고 학생 교육정보화 지원 (대전광역시교육청)"
type: "support-program"
domain: "local-government-supports"
basis_year: 2026
reviewed_at: "2026-07-11"
source_export: "korea-local-government-supports-ontology-2026.json"
source_urls: ["https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList/benefitSrvcDtl?svcSeq=2001602&bnefType=all&svcId=743000000012", "https://www.law.go.kr/법령/초ㆍ중등교육법/(21013,20250814)", "http://oneclick.neis.go.kr", "https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList"]
source_basis_dates: ["정부24 원문 수정일 2026-06-12", "수집일 2026-07-11", "2026-05-04 확인"]
tags: ["graph-materialized", "local-government-supports", "local-government-support", "gov24", "generated", "현금(감면), 현물", "custom-overlay"]
---

# 초·중·고 학생 교육정보화 지원 (대전광역시교육청)

대전광역시교육청 관할 지자체 지원금입니다. 저소득층 가정의 학생에게 PC, 인터넷 통신비를 지원해 온라인 학습 환경을 마련해줘요. 교육 격차 해소와 학업 지속을 돕는 제도입니다.

> Graph-only materialization입니다. 원본 데이터는 JSON export가 authoritative source입니다.

## Metadata
- Materialized ID: `support.local-gov.gov24.2001602`
- Type: `support-program`
- jurisdiction: 대전광역시교육청
- application_method: 온오프라인
- receiving_agency: 주민센터
- contact: 교육복지안전과(042-616-8805)
- source_modified_at: 2026-06-12
- source_collected_at: 2026-07-11

## Relations
- parents: [[30_Supports/LocalGovernment/지자체 지원금|지자체 지원금]]
- terms: [[40_Terms/지자체 지원금|지자체 지원금]]
- sources: [[90_Sources/정부24 보조금24 전체 혜택|정부24 보조금24 전체 혜택]]

## Status Check
- https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList/benefitSrvcDtl?svcSeq=2001602&bnefType=all&svcId=743000000012

## Source URLs
- https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList/benefitSrvcDtl?svcSeq=2001602&bnefType=all&svcId=743000000012
- https://www.law.go.kr/법령/초ㆍ중등교육법/(21013,20250814)
- http://oneclick.neis.go.kr
- https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList
