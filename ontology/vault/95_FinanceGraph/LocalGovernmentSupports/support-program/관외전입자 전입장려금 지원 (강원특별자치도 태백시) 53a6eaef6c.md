---
materialized_id: "support.local-gov.gov24.1003770"
title: "관외전입자 전입장려금 지원 (강원특별자치도 태백시)"
type: "support-program"
domain: "local-government-supports"
basis_year: 2026
reviewed_at: "2026-05-04"
source_export: "korea-local-government-supports-ontology-2026.json"
source_urls: ["https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList/benefitSrvcDtl?svcSeq=1003770&bnefType=all&svcId=422000000119", "https://www.law.go.kr/자치법규/태백시 인구늘리기 지원 조례/(2066,20221014)", "https://www.law.go.kr/자치법규/태백시 인구늘리기 지원 조례 시행규칙/(1061,20221014)", "https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList"]
source_basis_dates: ["정부24 원문 수정일 2026-04-29", "수집일 2026-05-04", "2026-05-04 확인"]
tags: ["graph-materialized", "local-government-supports", "local-government-support", "gov24", "generated", "현금", "custom-overlay"]
---

# 관외전입자 전입장려금 지원 (강원특별자치도 태백시)

강원특별자치도 태백시 관할 지자체 지원금입니다. 타 지역에서 전입한 주민의 안정적 정착을 위해 전입장려금을 지원하는 제도입니다. 
전입 후 일정 금액을 지급받아 초기 생활비 부담을 줄이고 지역사회 적응에 도움을 받을 수 있습니다.

> Graph-only materialization입니다. 원본 데이터는 JSON export가 authoritative source입니다.

## Metadata
- Materialized ID: `support.local-gov.gov24.1003770`
- Type: `support-program`
- jurisdiction: 강원특별자치도 태백시
- application_method: 오프라인
- receiving_agency: 동 행정복지센터, 주민센터
- contact: 태백시청 예산정책실(033-550-2441)
- source_modified_at: 2026-04-29
- source_collected_at: 2026-05-04

## Relations
- parents: [[30_Supports/LocalGovernment/지자체 지원금|지자체 지원금]]
- terms: [[40_Terms/지자체 지원금|지자체 지원금]]
- sources: [[90_Sources/정부24 보조금24 전체 혜택|정부24 보조금24 전체 혜택]]

## Status Check
- https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList/benefitSrvcDtl?svcSeq=1003770&bnefType=all&svcId=422000000119

## Source URLs
- https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList/benefitSrvcDtl?svcSeq=1003770&bnefType=all&svcId=422000000119
- https://www.law.go.kr/자치법규/태백시 인구늘리기 지원 조례/(2066,20221014)
- https://www.law.go.kr/자치법규/태백시 인구늘리기 지원 조례 시행규칙/(1061,20221014)
- https://plus.gov.kr/portal/benefitV2/benefitTotalSrvcList
