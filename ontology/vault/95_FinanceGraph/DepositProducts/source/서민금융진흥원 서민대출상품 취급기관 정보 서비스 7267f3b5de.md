---
materialized_id: "source.data.go.kr.kinfa-loan-handling-agencies"
title: "서민금융진흥원 서민대출상품 취급기관 정보 서비스"
type: "source"
domain: "deposit-products"
basis_year: null
reviewed_at: null
source_export: "korea-deposit-products-ontology-2026.json"
source_urls: ["https://www.data.go.kr/data/15074508/openapi.do"]
source_basis_dates: ["2026-07-03 확인"]
tags: ["graph-materialized", "deposit-products", "official-source", "finance-source"]
---

# 서민금융진흥원 서민대출상품 취급기관 정보 서비스

서민대출상품별 기관명, 법인번호, 금융기관주소, 상품명 등 취급기관 정보를 제공하는 API 후보입니다.

> Graph-only materialization입니다. 원본 데이터는 JSON export가 authoritative source입니다.

## Metadata
- Materialized ID: `source.data.go.kr.kinfa-loan-handling-agencies`
- Type: `source`

## Relations

## Source URLs
- https://www.data.go.kr/data/15074508/openapi.do
